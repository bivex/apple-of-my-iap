...
func (r *Renderer) RenderReceiptResponse(resp *ReceiptResponse) (string, error) {
	// Build the response in Apple's format
...
// Apple's format has some quirks:
// - Dates are in format "yyyy-MM-dd HH:mm:ss 'Etc/GMT'"
// - Dates also have millisecond versions like "purchase_date_ms"
// - Booleans are strings ("true"/"false") for is_trial_period
// - Quantity is a string ("1")
...
// formatDate formats a time in Apple's date format.
func formatDate(t time.Time) string {
	// Apple uses UTC/GMT for receipt dates
