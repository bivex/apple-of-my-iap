...
func (g *ReceiptGenerator) GenEncoding(plan *Plan, existingEncodings map[string]bool) string {
	for {
		id := uuid.New().String()
		// Use first 8 chars in format: planName_first4-second4
		parts := id[:8]
		first4 := parts[0:4]
		second4 := parts[4:8]
...
// If sub is provided, it's a renewal; otherwise it's a new subscription.
func (g *ReceiptGenerator) GenerateReceipt(plan *Plan, sub *Subscription) (string, *ReceiptInfo) {
	...
	if sub == nil {
		...
		receiptToken = "" // Will be set by caller via GenEncoding
	} else {
		// Renewal
